...
static uint32_t SFDP_maxEraseChipTime(void)
{
  ...
  // Guard against inconsistencies
  if(t_time == 0xFF && multiplier == 0xF) {
    return 30000;
  }
...
static uint32_t SFDP_getPageSize()
{
  ...
  //If this value is inconsistent, fall back to 256 bytes
  if(pageSize > 8192 || pageSize < 16) {
    pageSize = 256;
  }
...
BootloaderStorageImplementationInformation_t getDeviceInfo(void)
{
  ...
  flashInfo.wordSizeBytes = SFDP_getPageSize();
