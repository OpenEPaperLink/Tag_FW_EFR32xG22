...
// -----------------------------------------------------------------------------
#ifndef UCVAR026_DEBUG_PRINT
#define UCVAR026_DEBUG_PRINT 1
#endif

// -----------------------------------------------------------------------------
//                              Macros and Typedefs
// -----------------------------------------------------------------------------
#if UCVAR026_DEBUG_PRINT
...
//                                Global Variables
// -----------------------------------------------------------------------------
const oepl_display_driver_desc_t oepl_display_driver_ucvar026 =
{
  .init = &display_init,
  .draw = &display_draw
};
...
static void display_init(const oepl_display_parameters_t* display_params)
{
  DPRINTF("Initialising UC 2.6\" variant driver\n");
...
static void display_draw(void)
{
  DPRINTF("enter UC 2.6\" variant draw\n");
  display_reinit();
...
static void display_reinit(void)
{
  ...
  EMIT_INSTRUCTION_STATIC_DATA(0xB3, {0xE1});
  EMIT_INSTRUCTION_STATIC_DATA(0xB6, {0x51});
  EMIT_INSTRUCTION_STATIC_DATA(0x03, {0x30});
  EMIT_INSTRUCTION_STATIC_DATA(0xF3, {0x0A});
  EMIT_INSTRUCTION_STATIC_DATA(0xE9, {0x02});
  EMIT_INSTRUCTION_STATIC_DATA(0x00, {0x97, 0x09});
  EMIT_INSTRUCTION_STATIC_DATA(0x06, {0xE5, 0x35, 0x3C});
  EMIT_INSTRUCTION_STATIC_DATA(0x50, {0x57});
  EMIT_INSTRUCTION_STATIC_DATA(0x00, {0x10, 0x0C});
}
